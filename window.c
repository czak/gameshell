...
static void egl_init()
{
	...
	const EGLint context_attributes[] = {EGL_CONTEXT_MAJOR_VERSION, 2, EGL_NONE};
	...
	if (!egl.config || num_configs != 1)
		log_fatal("eglChooseConfig failed");

	egl.context = eglCreateContext(egl.display, egl.config, EGL_NO_CONTEXT, context_attributes);

	if (!egl.context)
		log_fatal("Failed to create EGL rendering context");

	window.wl_egl_window = wl_egl_window_create(window.wl_surface, window.width, window.height);
	window.egl_surface = eglCreateWindowSurface(egl.display, egl.config, window.wl_egl_window,
			NULL);

	if (!window.egl_surface)
		log_fatal("Failed to create EGL rendering surface");
		...
		void (*on_key)(int key), void (*on_close)())
{
	...
	if (!globals.wl_compositor)
		log_fatal("Failed to bind wl_compositor");
	if (!globals.wl_seat)
		log_fatal("Failed to bind wl_seat");
	if (!globals.zwlr_layer_shell_v1)
		log_fatal("Failed to bind zwlr_layer_shell_v1");
	if (!globals.wp_fractional_scale_manager_v1)
		log_fatal("Failed to bind wp_fractional_scale_manager_v1");
	if (!globals.wp_viewporter)
		log_fatal("Failed to bind wp_viewporter");
...
void window_show()
{
	window.zwlr_layer_surface_v1 =
			zwlr_layer_shell_v1_get_layer_surface(globals.zwlr_layer_shell_v1, window.wl_surface,
					NULL, ZWLR_LAYER_SHELL_V1_LAYER_OVERLAY, "gameshell");

	if (!window.zwlr_layer_surface_v1)
		log_fatal("Failed to create layer surface");
